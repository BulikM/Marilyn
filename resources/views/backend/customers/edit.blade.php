...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item "><a class="text-white text-decoration-none" href="{{route('users.index')}}">Customers</a>
    </li>
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{action('App\Http\Controllers\Backend\BCustomerController@update', $customer->id)}}"
                        ...
                        <div class="row row-cols-1 row-cols-lg-3">
                            @foreach($ReadOrShops as $ReadOrShop)
                                <div class="card col bg-none">
                                    <input type="radio" class="btn-check" value="{{$ReadOrShop->id}}" name="ReadOrShop"
                                           id="ReadOrShop{{$ReadOrShop->id}}" autocomplete="off" @checked($customer->read_or_shop_id == $ReadOrShop->id)>
                                    <label class="button btn-outline-purple"
                    ...
                    <hr>
                    <div class="float-end">
                        <a href="{{route('shipping-address.create', $customer->id )}}" class="text-capitalize btn text-decoration-underline"> Add New Addres</a>
                    </div>
                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-8 col-xs-8 address-display">
                                            <p class="text-capitalize">
                                                ...
                                                <br>
                                                {{--                                                    {{$s_address->street}} {{$s_address->number}} {{$S_address->bus ? $s_address->bus : " "}}--}}
                                                <br>
                                        ...
                                        <div class="col-sm-4 col-xs-4 address-controls">
                                            <p class="actions">
                                                ...
                                                |
                                            <form action="{{route('shipping-address.destroy', $s_address->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit">
                                                    Delete
                                                ...
                                                <a class="set-default-btn" rel="nofollow" data-method="post"
                                                   href="/account/addresses/4954897/assign_default">
                                                    <input type="radio" name="is_primary"
                                                           id="{{$s_address->is_primary}}" @checked($s_address->is_primary == 1)>
                                                    &nbsp; Default
                                                </a></p>
                                        </div>
                                    </div>
